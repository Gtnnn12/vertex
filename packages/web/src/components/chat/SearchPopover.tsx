      ...
      <div className="flex-1 min-w-0">
        ...
        {msg.attachments && msg.attachments.length > 0 && (
          <div className="flex items-center gap-1 mt-1 text-[11px] text-txt-tertiary">
            ...
            {msg.attachments.length} {msg.attachments.length === 1 ? t('attachment') : t('attachments')}
          </div>
        )}
      </div>
    </button>
      ...
      <div className="p-3 border-b border-white/[0.07]">
        ...
        {showFilters && (
          <div className="mt-2 grid grid-cols-2 gap-2">
            <div>
              <label className="text-[11px] text-txt-tertiary font-medium mb-1 block">{t('from')}</label>
              <input
                type="text"
                value={fromFilter}
            ...
            <div>
              <label className="text-[11px] text-txt-tertiary font-medium mb-1 block">{t('has')}</label>
              <select
                value={hasFilter}
            ...
            <div>
              <label className="text-[11px] text-txt-tertiary font-medium mb-1 block">{t('before')}</label>
              <input
                type="date"
                value={beforeFilter}
            ...
            <div>
              <label className="text-[11px] text-txt-tertiary font-medium mb-1 block">{t('after')}</label>
              <input
                type="date"
                value={afterFilter}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0">
        ...
        {!isSearching && results.length === 0 && (query || fromFilter || hasFilter || beforeFilter || afterFilter) && (
          <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
            ...
            <span className="text-txt-tertiary text-[13px]">{t('no_results')}</span>
          </div>
        )}

        {!query && !fromFilter && !hasFilter && !beforeFilter && !afterFilter && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
            <span className="text-txt-tertiary text-[13px]">{t('type_to_search')}</span>
          </div>
        )}
          ...
          <>
            ...
            {results.length < totalCount && (
              <div className="px-3 py-2 border-t border-white/[0.07]">
                ...
                >
                  {isSearching ? t('loading') : `${t('load_more')} (${totalCount - results.length} ${t('remaining')})`}
                </button>
              </div>
            )}
          </>
