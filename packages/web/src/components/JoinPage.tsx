...
export function JoinPage() {
  ...
  useEffect(() => {
    if (!rawInviteCode) {
      setPreviewError(t('no_invite_code_provided'));
      setIsLoadingPreview(false);
      return;
    }

    if (!parsed) {
      setPreviewError(t('invalid_invite_code'));
      setIsLoadingPreview(false);
      return;
    }
    ...
    const fetchPreview = async () => {
      try {
        ...
        setPreviewError(err instanceof Error ? err.message : t('failed_to_load_invite'));
      } finally {
        setIsLoadingPreview(false);
      }
    };
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (err instanceof NotConnectedError) {
        ...
        const msg = err instanceof Error ? err.message : t('failed_to_join_space');
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof DifferentPasswordError) {
        ...
        const msg = err instanceof Error ? err.message : t('failed_to_connect');
        if (msg.toLowerCase().includes('already a member')) {
          setPhase('already-member');
          setError('');
        } else {
  ...
  const handleFallbackLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : t('failed_to_log_in');
      if (msg.toLowerCase().includes('already a member')) {
        setPhase('already-member');
        setError('');
      } else {
    ...
    return (
      <div className="min-h-full flex items-center justify-center bg-surface-base relative">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-txt-tertiary">{t('loading_invite')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-full flex items-center justify-center bg-surface-base relative">
        ...
        <div className="w-full max-w-[480px] bg-surface-elevated rounded-md p-8 shadow-elevation-high relative z-10 text-center">
          ...
          <h1 className="text-xl font-bold text-txt-primary mb-2">{t('invalid_invite')}</h1>
          <p className="text-txt-secondary text-sm mb-6">
            {previewError || t('invite_link_invalid_or_expired')}
          </p>
          {token ? (
            <button
    ...
    <div className="min-h-full flex items-center justify-center bg-surface-base relative">
      ...
      <div className="w-full max-w-[480px] bg-surface-elevated rounded-md p-8 shadow-elevation-high relative z-10">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-txt-tertiary uppercase tracking-wide mb-1">{t('youve_been_invited_to_join')}</p>
          ...
          <div className="flex items-center justify-center gap-4 mt-3 text-xs text-txt-tertiary">
            <span className="flex items-center gap-1">
              ...
              {preview.memberCount} {preview.memberCount === 1 ? t('member') : t('members')}
            </span>
          ...
          <>
            {token && user ? (
              ...
              <div className="space-y-3">
                ...
                >
                  {isJoining ? t('joining') : `${t('join_as')} ${user.displayName || user.username}`}
                </button>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-txt-tertiary">{t('or')}</span>
                  <div className="flex-1 h-px bg-border-soft" />
                </div>
        ...
        {phase === 'connect' && (
          <form onSubmit={handleConnect}>
            ...
            <p className="text-txt-tertiary text-xs mb-4">
              {t('connecting_to')} <span className="text-txt-secondary font-medium">{hostDisplay}</span>
            </p>
            <div className="mb-4">
              <label className="block text-xs font-bold text-txt-secondary uppercase mb-1.5">
                {t('password')}
              ...
              <p className="text-xs text-txt-tertiary mt-1">
                {t('password_verified_locally_hint')}
              </p>
            </div>
            <div className="flex gap-2">
              ...
              >
                {isJoining ? t('connecting') : t('connect_and_join')}
              </button>
            </div>
          </form>
        )}
        ...
        {phase === 'fallback' && (
          <form onSubmit={handleFallbackLogin}>
            <div className="mb-3 p-2 bg-accent-amber/10 border border-accent-amber/30 rounded text-xs text-accent-amber">
              {t('account_exists_different_password').replace('{host}', hostDisplay)}
            </div>
            <div className="mb-4 space-y-3">
              <div>
                <label className="block text-xs font-bold text-txt-secondary uppercase mb-1.5">{t('username')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-txt-secondary uppercase mb-1.5">{t('password_for_this_instance')}</label>
                <input
                  type="password"
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-accent-primary hover:bg-accent-primary/80 text-white font-medium rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isJoining ? t('logging_in') : t('login_and_join')}
              </button>
            </div>
          </form>
        )}
...
function AlreadyMemberCard({ spaceName, spaceId, navigate }: { spaceName: string; spaceId: string; navigate: (path: string) => void }) {
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-txt-primary font-medium mb-1">{t('already_in_space').replace('{space}', spaceName)}</p>
      <p className="text-txt-tertiary text-xs mb-4">{t('redirecting_you_now')}</p>
      <button
        onClick={() => navigate(`/channels/${spaceId}`)}
        className="px-6 py-2.5 bg-accent-primary hover:bg-accent-primary/80 text-white font-medium rounded transition-colors"
      >
        {t('go_to_space').replace('{space}', spaceName)}
      </button>
    </div>
  );
}
