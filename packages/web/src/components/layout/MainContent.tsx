...
export function MainContent() {
  ...
  if (showDms || isExplorePage || !currentSpaceId) {
    ...
    const dmInputPlaceholder = isGroupDm && dmChannel
      ? `${t('message')} ${formatDmInputLabel(dmChannel, authUser)}`
      : dmChannel
        ? `${t('message')} @${dmName}`
        : undefined;
    ...
    return (
      <div className="flex-1 flex flex-col bg-surface-chat relative">
        ...
        <div className="flex-1 flex items-center justify-center text-txt-tertiary">
          <p>{t('select_text_or_voice_channel_hint')}</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-[10px]">
            ...
            {connectionError ? (
              ...
              <>
                ...
                <span className="text-xs text-txt-tertiary ml-1">{participants.length} {t('connected')}</span>
              </>
            ) : (
  ...
  return (
    <div className="flex-1 flex flex-col bg-surface-chat min-w-0 relative">
      <div className="h-14 px-5 flex items-center justify-between border-b border-border-hard flex-shrink-0 z-10 bg-surface-chat">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button className="w-8 h-8 flex items-center justify-center text-txt-tertiary hover:text-txt-primary transition-colors rounded-[6px] hover:bg-interactive-hover" title={t('notification_settings')}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
