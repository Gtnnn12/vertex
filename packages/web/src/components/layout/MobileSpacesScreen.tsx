    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-surface-channel">
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {/* Bulk permissions — fixed at the top, same section-header style */}
          {canManageRoles && (
            <div className="mt-3 mb-1">
              <button
                data-context-menu
                onClick={() => openModal('bulkPermissions')}
                className="flex items-center gap-1 px-1 py-1 w-full text-left group"
              >
                <svg
                  className="w-3 h-3 text-txt-tertiary group-hover:text-txt-primary transition-colors"
                  fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
                >
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 10.5v3.75m-9.303 3.376C3.804 19.496 7.583 21.75 12 21.75s8.196-2.254 9.303-5.124A41.219 41.219 0 0112 15.75c-3.246 0-6.386.66-9.303 1.876zM6.75 6.75V4.5A2.25 2.25 0 019 2.25h6a2.25 2.25 0 012.25 2.25v2.25" />
                  <rect x="3" y="10.5" width="18" height="9" rx="2" />
                </svg>
                <span className="text-[11px] font-semibold uppercase tracking-wide text-txt-tertiary group-hover:text-txt-primary transition-colors">
                  {t('permissions')}
                </span>
              </button>
            </div>
