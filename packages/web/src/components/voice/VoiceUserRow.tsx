      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        {showServerMicIcon && (
          <span
            title={
              isPermissionMuted
                ? t('muted_no_speak_permission')
                : isServerMuted
                  ? t('space_muted')
                  : t('muted_space_deafened')
            }
          >
        ...
        {isScreenSharing && (
          <span className="bg-accent-rose text-white text-[9px] font-bold px-1 rounded leading-[14px]">{t('live')}</span>
        )}
        {/* Locally muted — volume X icon */}
        {isLocallyMuted && (
          <span title={t('locally_muted')}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-txt-tertiary">
