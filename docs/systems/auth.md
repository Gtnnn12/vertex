...
```typescript
interface AuthState {
  token: string | null;        // Persisted in localStorage as 'VERTEX_token'
...
**Initialization:** `token` is read from `localStorage.getItem('VERTEX_token')` on store creation.
