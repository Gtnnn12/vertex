#!/usr/bin/env bash
# ============================================================
# VERTEX — Production Installer
# ============================================================
# Sets up VERTEX on a Linux server in one of three deployment modes,
...
#             Manager, SWAG…). VERTEX is published on 127.0.0.1:APP_PORT and
...
#     ./install.sh
#
#   VERTEX_BUILD=true ./install.sh  Force a local from-source build instead of
...
if [[ "$(uname -s)" != "Linux" ]]; then
  error "VERTEX production deployment requires Linux."
...
# In use by something OTHER than our own VERTEX container?
...
choose_mode_interactively() {
  ...
  if [[ "$port80_free" == true && "$port443_free" == true ]]; then
    ...
    echo ""
    echo "  How do you want to expose VERTEX?"
    ...
    info "That's fine — run VERTEX behind what already owns those ports:"
    echo ""
...
if [[ -z "${INSTANCE_NAME:-}" ]]; then
  read -rp "Instance name [${existing_name:-VERTEX}]: " INSTANCE_NAME || INSTANCE_NAME=""
  INSTANCE_NAME="${INSTANCE_NAME:-${existing_name:-VERTEX}}"
fi
...
if $DOCKER volume inspect VERTEX-data &>/dev/null 2>&1; then
  if [[ ! -f ./data/VERTEX.db ]]; then
    step "Migrating data from legacy Docker volume"
    info "Copying VERTEX-data volume contents to ./data ..."
    $DOCKER run --rm \
      -v VERTEX-data:/source:ro \
      -v "$(pwd)/data:/target" \
      alpine sh -c "cp -a /source/. /target/"
    success "Data migrated from VERTEX-data volume to ./data"
    info "The old volume is still intact. Remove it with: docker volume rm VERTEX-data"
  fi
fi

# Also clean up legacy external network if it exists (no longer needed)
if $DOCKER network inspect VERTEX-net &>/dev/null 2>&1; then
  info "Legacy VERTEX-net network detected. It is no longer needed."
  info "Remove after verifying: docker network rm VERTEX-net"
fi

# ── Phase 6: Build & Deploy ────────────────────────────────

step "Deploying VERTEX"
...
build_from_source() {
  if [[ -n "$BUILD_COMMIT" ]]; then
    info "Building VERTEX image from commit ${BUILD_COMMIT} (first build takes a few minutes)..."
  else
    info "Building VERTEX image (first build takes a few minutes)..."
  fi
  $COMPOSE build --build-arg VERTEX_COMMIT="$BUILD_COMMIT"
}
...
# if the operator forces a build (VERTEX_BUILD=true — e.g. running a fork).
if [[ "${VERTEX_BUILD:-false}" == "true" ]]; then
...
info "Waiting for VERTEX to become healthy..."
healthy=false
for i in $(seq 1 60); do
  status=$($DOCKER inspect VERTEX --format '{{.State.Health.Status}}' 2>/dev/null || echo "waiting")
...
if [[ "$healthy" == true ]]; then
  success "VERTEX is healthy"
else
  warn "Health check hasn't passed yet. Check logs: docker compose logs VERTEX"
fi

# ── Phase 7: Set instance name ──────────────────────────────

if [[ "$healthy" == true && -n "$INSTANCE_NAME" && "$INSTANCE_NAME" != "VERTEX" ]]; then
  ...
  $DOCKER exec -e BS_INSTANCE_NAME="$INSTANCE_NAME" -w /app/packages/server VERTEX node -e '
    const Database = require("better-sqlite3");
    const db = new Database("/app/data/VERTEX.db");
...
if [[ "$DEPLOY_MODE" == "allinone" ]]; then
  if [[ "$healthy" == true ]]; then
    ...
    if [[ "$https_status" == "live" ]]; then
      success "HTTPS is live — a valid TLS certificate is installed and VERTEX is serving over it."
    else
  ...
  if [[ "$healthy" == true ]]; then
    ...
    if [[ "$app_reachable" == "yes" ]]; then
      success "VERTEX is answering on http://127.0.0.1:${APP_PORT_FINAL} — point your $( [[ "$DEPLOY_MODE" == tunnel ]] && echo tunnel || echo 'reverse proxy') at it."
    else
      warn "Could not reach http://127.0.0.1:${APP_PORT_FINAL}/api/health yet — check: docker compose logs VERTEX"
    fi
  fi
fi
...
if [[ "$DEPLOY_MODE" == "allinone" ]]; then
  ...
  echo ""
  echo -e "  VERTEX listens on ${BOLD}127.0.0.1:${APP_PORT_FINAL}${NC} (loopback only — never expose it directly)."
  ...
  echo ""
  echo -e "  VERTEX listens on ${BOLD}127.0.0.1:${APP_PORT_FINAL}${NC} (loopback only). Your tunnel fronts it."
  ...
  echo -e "  If you need voice, run VERTEX behind a reverse proxy (proxy mode) with the"
